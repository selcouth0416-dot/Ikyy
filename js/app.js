(() => {
  ...
  function renderVerbs() {
    ...
    const list = VERBS.filter((v) => {
      const okLv = state.filter === "semua" || v.lv === state.filter ||
        ...
        (state.filter === "unreg" && (v.typ === "unregelmÃ¤Ãig" || v.typ === "gemischt"));
    ...
    `).join("") || `<p class="muted">Tidak ditemukan. Coba cari di kotak âkata kerja belum diketahuiâ di atas.</p>`;
  ...
  function guessRegular(inf) {
    ...
    const e = /[td]$/.test(stem) || /[mn][^aeiouÃ¤Ã¶Ã¼hlr]$/.test(stem);
    const st = e ? "est" : "st";
    const t = e ? "et" : "t";
    return {
      inf,
      id: "(perkiraan regular â periksa kamus)",
      en: "",
      lv: "?",
      ...
      ex: [{ de: `Ich ${stem}e das.`, id: "Kalimat contoh generik â ganti sesuai arti." }],
      guessed: true,
    };
  }
  ...
  function openVerb(inf, guessed) {
    ...
    $("#modal-body").innerHTML = `
      <div class="row" style="justify-content:space-between">
        <div>
          <div class="eyebrow">${v.lv} Â· ${v.typ} Â· Perfekt dengan ${v.aux}</div>
          <h2>${v.inf}</h2>
          <p class="muted">${v.id}${v.en ? " Â· " + v.en : ""}</p>
        </div>
        <button class="icon-btn" id="say-inf" title="Dengar">âª</button>
      </div>
      ${v.guessed ? `<div class="note">Verba ini tidak ada di daftar A1âA2. Ini tebakan pola regular. Cek di
      ...
      <table class="conj-table">
        <tr><th>Pronomina</th><th>PrÃ¤sens</th></tr>
      ...
      <p style="margin-top:14px"><strong>PrÃ¤teritum:</strong> ${v.praet} &nbsp; Â· &nbsp; <strong>Partizip II:</strong> ${v.aux} ${v.part}</p>
      <h3 style="margin-top:22px">Contoh kalimat</h3>
      ${(v.ex || []).map((e, i) => `
        <div class="example">
          <div class="row" style="justify-content:space-between">
            <div class="de">${e.de}</div>
            <button class="icon-btn say-ex" data-i="${i}">âª</button>
          </div>
  ...
  const DRILL = [
    { q: "Ich ___ aus Indonesien.", opts: ["komme", "kommt", "kommen", "kommst"], a: 0, tip: "ich + PrÃ¤sens regular/kuat tanpa ubah vokal." },
    { q: "Wie ___ du?", opts: ["heiÃt", "heiÃen", "heiÃe", "heiÃt ihr"], a: 0, tip: "du-form dari heiÃen." },
    ...
    { q: "Der Zug fÃ¤hrt um acht ___.", opts: ["ab", "an", "auf", "aus"], a: 0, tip: "abfahren â awalan terpisah di akhir." },
    ...
    { q: "Ich bleibe zu Hause, ___ ich mÃ¼de bin.", opts: ["weil", "dass", "und", "oder"], a: 0, tip: "weil + verba di akhir." },
    { q: "Gestern ___ ich im BÃ¼ro gewesen.", opts: ["bin", "habe", "war", "seid"], a: 0, tip: "sein + gewesen." },
    ...
    { q: "Ich gehe ___ die KÃ¼che.", opts: ["in", "im", "an", "auf"], a: 0, tip: "in + Akkusativ untuk gerak masuk." },
    { q: "___ Sie das bitte wiederholen?", opts: ["KÃ¶nnten", "Kann", "MÃ¼sst", "Wollt"], a: 0, tip: "Bentuk sopan A2." },
    { q: "Am ___ gehe ich arbeiten.", opts: ["Montag", "Mai", "acht Uhr", "Sommer"], a: 0, tip: "am + hari." },
    { q: "Ich mÃ¶chte einen ___.", opts: ["Kaffee", "Kaffees", "dem Kaffee", "des Kaffees"], a: 0, tip: "Akkusativ setelah mÃ¶chte + artikel ein." },
    { q: "Sie ___ nicht rauchen.", opts: ["darf", "darfst", "dÃ¼rft", "kannst"], a: 0, tip: "er/sie/es dari dÃ¼rfen = darf." },
    { q: "Hast du den Film ___?", opts: ["gesehen", "gesehen haben", "sah", "seht"], a: 0, tip: "sehen â gesehen." },
  ...
  function startVocab() {
    ...
    const items = pool.map((v) => {
      ...
      return { q: `Apa arti â${v.inf}â?`, opts, a: opts.indexOf(v.id), speak: v.inf, tip: v.ex?.[0]?.de || "" };
    });
  ...
  function startConj() {
    const pool = shuffle(VERBS.filter((v) => v.praes?.du && v.praes.du !== "â")).slice(0, 10);
    const items = pool.map((v) => {
      ...
      return { q: `Bentuk du dari â${v.inf}â?`, opts, a: opts.indexOf(correct), speak: v.inf, tip: `ich ${v.praes.ich}, er ${v.praes.er}` };
    });
  ...
  function renderQuiz() {
    ...
    box.innerHTML = `
      <div class="muted">Soal ${qz.i + 1} / ${qz.items.length} Â· skor ${qz.score}</div>
      <div class="progress"><span style="width:${(qz.i / qz.items.length) * 100}%"></span></div>
      <div class="row"><div class="q-text">${item.q}</div>
        ${item.speak ? `<button class="icon-btn" id="q-speak">âª</button>` : ""}</div>
      <div class="options">
  ...
  function bind() {
    ...
    $("#lookup-btn").onclick = () => {
      ...
      if (!raw) return;
      const hit = VERBS.find((v) => v.inf === raw || v.inf.replace("Ã", "ss") === raw);
